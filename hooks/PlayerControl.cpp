...
void dPlayerControl_FixedUpdate(PlayerControl* __this, MethodInfo* method) {
	try {
		...
		if ((IsInGame() || IsInLobby())) {
			...
			if ((State.Wallhack || State.IsRevived) && __this == *Game::pLocalPlayer && !State.FreeCam && !State.playerToFollow.has_value()) {
...
void dPlayerControl_HandleRpc(PlayerControl* __this, uint8_t callId, MessageReader* reader, MethodInfo* method) {
	try {
		...
		if ((State.DisableMeetings && (callId == 11 || callId == 14)) || (State.DisableSabotages && (callId == 27 || callId == 28)))
...
/*void dKillButton_CheckClick(KillButton* __this, PlayerControl* target, MethodInfo* method) {
	if (target == nullptr || GetPlayerData(target)->fields.IsDead || target == *Game::pLocalPlayer)
		return;
	float maxDistance = State.ModifyKillDistance ? (float)(State.KillDistance + 1) : (float)(GameOptions().GetInt(app::Int32OptionNames__Enum::KillDistance) + 1);
	if (State.InfiniteKillRange)
		maxDistance = FLT_MAX;
	if (Vector2_Distance(PlayerControl_GetTruePosition(*Game::pLocalPlayer, NULL), PlayerControl_GetTruePosition(target, NULL), NULL) > maxDistance)
		return;
	KillButton_CheckClick(__this, target, method);
}*/