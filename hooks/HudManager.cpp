...
void dVersionShower_Start(VersionShower* __this, MethodInfo* method) {
	...
	const auto& versionText = !State.DisableSMAU ? std::format("<size=75%>{} ~ <#0f0>SickoMode</color><#f00>AU</color> <#fb0>v2.0</color></size>",
		convert_from_string(app::TMP_Text_get_text((app::TMP_Text*)__this->fields.text, nullptr))) : convert_from_string(app::TMP_Text_get_text((app::TMP_Text*)__this->fields.text, nullptr));
...
void dPingTracker_Update(PingTracker* __this, MethodInfo* method) {
	...
	if (!State.DisableSMAU) {
		...
		std::string pingText = std::format("{}\n<size=50%><#0f0>SickoMode</color><#f00>AU</color> <#fb0>v2.0</color>{}{}{}{}</size>", ping, hostText, noClip, freeCam, spectating);
