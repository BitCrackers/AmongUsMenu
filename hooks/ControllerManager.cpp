...
void dControllerManager_CloseOverlayMenu(ControllerManager* __this, String* menuName, MethodInfo* method)
{
	...
	if (convert_from_string(menuName) == "MeetingHub(Clone)" &&
		(convert_from_string(currentMenuName) != "MeetingHub(Clone)" || !uiStack.empty()))
	{
		LOG_INFO("Close UI stack before MeetingHub to fix Innersloth movement bug");
		for (ControllerUiElementsState* uiElement : uiStack)
			ControllerManager_CloseOverlayMenu(__this, uiElement->fields.MenuName, method);
