...
void dMeetingHud_Update(MeetingHud* __this, MethodInfo* method) {
	...
	for (size_t i = 0; i < playerStates->max_length; i++) {
		...
		if (playerVoteArea && playerData)
		{
			...
			{
				State.events[playerVoteArea->fields.TargetPlayerId][EVENT_VOTE].emplace_back(std::make_unique<CastVoteEvent>(GetEventPlayer(playerData).value(), GetEventPlayer(GetPlayerDataById(playerVoteArea->fields.VotedFor))));
				State.consoleEvents.emplace_back(std::make_unique<CastVoteEvent>(GetEventPlayer(playerData).value(), GetEventPlayer(GetPlayerDataById(playerVoteArea->fields.VotedFor))));
				State.flatEvents.emplace_back(std::make_unique<CastVoteEvent>(GetEventPlayer(playerData).value(), GetEventPlayer(GetPlayerDataById(playerVoteArea->fields.VotedFor))));
