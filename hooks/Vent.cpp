...
void dVent_EnterVent(Vent* __this, PlayerControl* pc, MethodInfo * method) {
	State.events[pc->fields.PlayerId][EVENT_VENT].push_back(new VentEvent(GetEventPlayer(pc), VENT_ENTER));
	State.consoleEvents.push_back(new VentEvent(GetEventPlayer(pc), VENT_ENTER));
	Vent_EnterVent(__this, pc, method);
}

void dVent_ExitVent(Vent* __this, PlayerControl* pc, MethodInfo * method) {
	State.events[pc->fields.PlayerId][EVENT_VENT].push_back(new VentEvent(GetEventPlayer(pc), VENT_EXIT));
	State.consoleEvents.push_back(new VentEvent(GetEventPlayer(pc), VENT_EXIT));
