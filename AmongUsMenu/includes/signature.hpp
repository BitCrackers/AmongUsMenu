...
class SignatureScan
{
	...
	uintptr_t FindSignatures(const char* signature, size_t index) {
		...
		K32GetModuleInformation(GetCurrentProcess(), GetModuleHandleA("GameAssembly.dll"), &moduleInfo, sizeof(MODULEINFO));
...
public:
	SignatureScan(const char* signature, size_t index) {
		pointer = FindSignatures(signature, index);
	}
	SignatureScan(const char* signature) : SignatureScan(signature, 0) {}
	operator T* () {
		return (T*)pointer;
	}
	operator uintptr_t () {
